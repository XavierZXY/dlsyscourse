...
class BatchNorm1d(Module):
    def __init__(self, dim, eps=1e-5, momentum=0.1, device=None, dtype="float32"):
        ...
        self.running_var = init.ones(self.dim, device=device, dtype=dtype, requires_grad=True)
...
class Dropout(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # NOTE: 1 - self.p
        mask = init.randb(*x.shape, p=1-self.p, device=x.devicem,dtype=x.dtype)
        if self.trainging:
            x_mask = x * mask
...
class Embedding(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        ### BEGIN YOUR SOLUTION
        one_hot_vectors = self.ont_hot = init.one_hot(self.num_embeddings, x, device=x.device, dtype=x.dtype)
