...
class DataLoader:
    ...
    """
    NOTE: 
    Python会首先调用该对象的 `__iter__` 方法来获取一个迭代器对象，
    然后不断调用迭代器对象的 `__next__` 方法来获取下一个批次的数据，直到遇到 StopIteration 异常。
    """
    def __iter__(self):
        ### BEGIN YOUR SOLUTION
        if self.shuffle:
...
class CIFAR10Dataset(Dataset):
    def __init__(
        ...
        ### BEGIN YOUR SOLUTION
        import pickle
        ### pickle模块的作用是把Python对象直接保存到文件里，
        ### 而不需要先把它们转化为字符串再保存，也不需要用底层的文件访问操作，
        ### 直接把它们写入一个二进制文件里。 
        if train:
